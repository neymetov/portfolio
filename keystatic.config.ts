      ...
      schema: {
        ...
        */
        ...
        /*
          ...
          Обложка может быть живой: картинка тогда работает постером — её видно, пока
          грузится видео, и она же остаётся у тех, кто выключил анимацию в системе.
          Видео берём вместо GIF: тот же ролик весит в разы меньше и не теряет цвета.
        */
        listCover: fields.conditional(
          fields.checkbox({ label: 'Своя обложка для списка работ' }),
          {
            false: fields.empty(),
            true: fields.object({
              image: image('Обложка (она же постер видео)', 'works'),
              video: fields.file({
                label: 'Видео вместо картинки (необязательно)',
                description: 'MP4 (H.264) или WebM без звука. Проигрывается по кругу.',
